...
function writeBackend(options, backendPath) {
  ...
  writeFile(path.join(backendPath, '.env'), generateBackendEnv(options, { dbPassword: options.dbPassword }));
  ...
  if (isFullstack || database !== 'none') {
    const rootEnv = (password) => `# Overrides for docker-compose.yml. Application configuration lives in
