...
const { generateRootPackageJson, generateBackendPackageJson } = require('./generators/manifest');
const {
...
const { DB_TEMPLATE_DIR } = require('./constants');
...
function writeSharedRoot(options) {
  ...
  copyDir(path.join(TEMPLATES, 'shared'), targetPath, { skip: ['gitignore'] });
...
function installDependencies(targetPath, { onWarn }) {
