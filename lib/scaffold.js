 ...
 */
/**
 * A unique database password per generated project.
 *
 * The alternative — a literal like `postgres` baked into docker-compose.yml —
 * is what GitGuardian flags, and rightly: it teaches the pattern, and a stack
 * deployed as-is runs on a password every reader already knows. base64url keeps
 * it shell- and URL-safe so it can sit in a DATABASE_URL unescaped.
 */
...
function scaffold(options, hooks = {}) {
  // One credential per project, shared by compose and the backend env.
  options.dbPassword = options.dbPassword || generateDbPassword();
...
function writeBackend(options, backendPath) {
  ...
  // .env.example is committed, so it must never carry the real credential.
  writeFile(
    path.join(backendPath, '.env.example'),
    generateBackendEnv(options, { dbPassword: 'replace-with-your-own' })
  );
  writeFile(
    path.join(backendPath, '.env'),
    generateBackendEnv(options, { dbPassword: options.dbPassword })
  );
  ...
  if (backend === 'express' && !isFullstack) {
    ...
    copyFile(
      path.join(TEMPLATES, 'shared', 'brand', 'logo.png'),
      path.join(backendPath, 'src', 'public', 'logo.png')
    );
  ...
  // Compose exists whenever there is a database or a second service, and it
  // reads its values from a root .env.
  if (isFullstack || database !== 'none') {
    const rootEnv = (
      password
    ) => `# Overrides for docker-compose.yml. Application configuration lives in
# ${isFullstack ? 'backend/.env and frontend/.env' : '.env in this directory'}.
#
# This file is gitignored. .env.example is the committed copy and carries a
# placeholder instead of the real credential.
BACKEND_PORT=3000
${isFullstack ? 'FRONTEND_PORT=5173\n' : ''}${
      database === 'postgres'
        ? `POSTGRES_PORT=55432

# Generated for this project. Compose refuses to start without it, so a
# deployment cannot fall back to a guessable default.
...
# Generated for this project. Without these the mongo container would start
# with authentication disabled and accept any connection.
MONGO_USER=app
...
`
    ...
    writeFile(path.join(targetPath, '.env.example'), rootEnv('replace-with-your-own'));
    writeFile(path.join(targetPath, '.env'), rootEnv(options.dbPassword));
  }
}
...
/** Vite and Next both serve everything under public/ from the site root. */
function writeBrandAsset(frontendPath) {
  copyFile(
    path.join(TEMPLATES, 'shared', 'brand', 'logo.png'),
    path.join(frontendPath, 'public', 'logo.png')
  );
}
...
function writeSharedRoot(options) {
  ...
  copyDir(path.join(TEMPLATES, 'shared'), targetPath, {
    skip: ['gitignore', '.nvmrc', 'brand'],
  });
