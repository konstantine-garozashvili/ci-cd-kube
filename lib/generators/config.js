...
#
# The host port is 55432, not 5432, so the container cannot collide with a
# PostgreSQL already installed on the machine — Laragon, Postgres.app, a system
# service. On Windows a second listener can bind the same port silently, and the
# app then authenticates against the wrong server; a distinct port avoids that
# entirely. Inside compose the service still listens on 5432.
DATABASE_URL=postgresql://postgres:${dbPassword}@localhost:55432/app_db?schema=public
`
      : database === 'mongodb'
        ? `
# Database — matches the mongodb service in docker-compose.yml. The container
# runs with authentication enabled, so the credentials are required and are
# authenticated against the admin database.
#
# The host port is 57017, not 27017, so the container cannot collide with a
# MongoDB already installed on the machine. Inside compose it still uses 27017.
MONGODB_URI=mongodb://app:${dbPassword}@localhost:57017/app_db?authSource=admin
`
        : '';
