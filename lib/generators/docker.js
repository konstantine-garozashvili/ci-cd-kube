...
/** Node major for the base image: whatever was resolved, else the baseline. */
const nodeMajor = (options) => (options.node && options.node.major) || NODE_VERSION;
...
function generateBackendDockerfile(options) {
  ...
  const NODE_VERSION = nodeMajor(options);
