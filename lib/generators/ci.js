...
# CI runs on every pull request and every push to main. Publishing is gated
# separately below: images are only pushed from main or a version tag, never
# from a pull request, because a PR branch is untrusted code.
on:
  pull_request:
    branches: [main]
  push:
...
# Nothing is granted by default; each job requests the minimum it needs.
permissions: {}
...
jobs:
  ...
  quality-gate:
    ...
    permissions:
      contents: read
      # gitleaks lists the pull request's commits through the API, which needs
      # this scope. Without it the action fails with "Resource not accessible
      # by integration" on every pull request.
      pull-requests: read
${servicesBlock(database)}
    steps:
      ...
      # Repos owned by a GitHub *organization* need a free licence key from
      # gitleaks.io, added as a GITLEAKS_LICENSE secret. Personal accounts need
      # nothing — the variable is simply empty and the action runs unlicensed.
...
${testEnvBlock(database, true)}
  ...
  #
  # This is the CD half. The build and CVE scan run for pull requests too, so a
  # broken Dockerfile is caught before merge, but the publish step is skipped
  # unless the run came from main or a version tag.
  # =========================================================================
  docker:
