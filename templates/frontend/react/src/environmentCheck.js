 ...
 *
 * Framework-free on purpose: React, Vue and Next all import this same file.
 */

/** One row per thing a reader would want confirmed. */
export async function runEnvironmentCheck(apiBase = '') {
  const rows = [];
  let ok = true;
  ...
  try {
    ...
    rows.push({ label: 'API server', ok: false, detail: `unreachable — ${err.message}` });
    ok = false;
  }
  ...
  try {
    ...
    const db = body.checks && body.checks.database;
    if (db) {
    ...
    const mem = body.checks && body.checks.memory;
    if (mem) {
      rows.push({
        label: 'Memory',
        ok: true,
    ...
    rows.push({ label: 'Readiness probe', ok: false, detail: `unreachable — ${err.message}` });
    ok = false;
  }
  ...
  try {
    ...
    rows.push({ label: 'API routes', ok: false, detail: `unreachable — ${err.message}` });
    ok = false;
  }
